...
@Component({
  ...
  templateUrl: './tntp-content.page.html',
  styleUrls: ['./tntp-content.page.scss'],
})
export class TntpContentPage implements OnInit {
  ques:any;
  quesValue:any;
  extra_field:any=[];
  extra_img:any=[]
  ...
  this.ionicStorageService.getData('pedagogy_info').then(Response=>{
    if(Response){
      ...
      if(this.last_saved_data.hasOwnProperty('pedagogy_info')){
        ...
        this.questionForm.value.pedagogy_info.forEach((value,index)=>{  // For IOnic Selectable onChange call
