...
@Component({
  ...
  templateUrl: './record-verification.page.html',
  styleUrls: ['./record-verification.page.scss'],
})
export class RecordVerificationPage implements OnInit {
  ...
  constructor(private api:ApiService,private fb:FormBuilder,private loadingCtrl:LoadingController,
    ...
    //  const alert= this.toast.presentAlertConfirm().then(res=>{
    //     //alert.present()
    //     // alert.onDidDismiss
    //   console.log(res)
    //   reject();
  ...
  //     return new Promise((resolve,reject)=>{
  //       const alert= this.toast.setAlert("Data will be Lost","Are you sure want to Go Back")
  //        alert.present();
...
//    }
  ...
  valueChanges(event,name,i){
        ...
        let value:any=[];
        //FOR THE ARRAY OF ANSWERS LIKE CHECKBOX
        ...
        //FOR THE SINGLE ANSWER LIKE INPUT,RADIO
        else{
          value=name.answer_key.filter(val=>val.ans == event)
       ...
       //FOR CREATE AND ADDING THE SUB FIELDS
        let sub_field_template
        if(value.length>0){
        ...
        if(typeof(event) == 'object'){
          ...
          //FOR NEXT FIELD SINGLE FIELD GENERATION
           if(value.length>0){
   if(value[0].next_filed == 'freetext'){
    // console.log("free text",value)
     this.extra_field[i]=value[0]
  ...
  onSubmit(){
   ...
   records.pedagogy_info.forEach(res=>{
    ...
    //this.navCtrl.setDirection('root')
    // this.router.navigate(['page-route','student-list'],navigationExtras)
